...
<body>
    ...
    <script src="./assets/iklan/iklan.js">
        function addHist(el) {
            const id = el.getAttribute('data-id');
            const jududl = el.getElementByClassName('card-head')[0].innerText
            ...
            if (localStorage.getItem('arr0fHyst') !== null){
                let histories = JSON.parse(localStorage.arr0fHyst)
                histories.push('hyst'+id)
                histories=[...new Set(histories)]

                localStorage.setItem('arr0fHyst', JSON.stringify(histories))
            }else{
                localStorage.setItem('arr0fHyst', JSON.stringify(['hyst'+id]))
            }
